import { PlayCircle, Clock, Flame, TrendingUp } from "lucide-react";
...
export default async function ParentStats({ childId }: ParentStatsProps) {
  ...
  const statsItems = [
    ...
    { label: "Chuỗi rèn luyện", value: `${stats.streak} Ngày`, icon: Flame, color: "text-amber-600 dark:text-amber-500", bg: "bg-amber-100 dark:bg-amber-500/10" },
  ];

  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {statsItems.map((stat, i) => (
        <div key={i} className="bg-white dark:bg-zinc-900/50 backdrop-blur border border-zinc-200 dark:border-zinc-800 p-5 rounded-2xl hover:border-blue-200 dark:hover:border-zinc-700 transition-colors shadow-sm dark:shadow-none">
