...
export default function RecommendationCard({ rec, index }: Props) {
  ...
  return (
    <div
      id={`rec-card-${index}`}
      className="group bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[2rem] p-8 shadow-sm hover:shadow-md hover:border-amber-200 dark:hover:border-amber-800/50 transition-all duration-300 flex flex-col gap-6"
    >
      {/* ── Top Header Row (Image + Title/Badges) ── */}
      <div className="flex flex-col md:flex-row gap-6 items-start">
        {/* Thumbnail area with perfect aspect ratio */}
        <div className="w-full md:w-56 h-32 relative overflow-hidden rounded-2xl bg-zinc-100 dark:bg-zinc-800 flex-shrink-0">
          {rec.thumbnailUrl ? (
            <img
          ...
          <div className="absolute bottom-3 left-4">
            <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border backdrop-blur-md ${typeColor}`}>
              {typeLabel}
            </span>
          </div>
        </div>

        {/* Title, Level & Badges next to image */}
        <div className="flex-1 space-y-3">
          <div>
            <span className="inline-block mb-1 text-[10px] font-black text-zinc-400 uppercase tracking-[0.15em]">
              Gợi ý #{index + 1}
            </span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-black border ${p.color}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${p.dot}`} />
              {p.label}
            </span>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-zinc-50 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-100 dark:border-zinc-700">
              <Target size={11} />
              {rec.targetSkill}
            </span>
          </div>
      ...
      {/* ── Bottom Content Grid (Clinical details) ── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-zinc-100 dark:border-zinc-800 pt-6">
