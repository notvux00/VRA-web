...
import { 
  Settings2, Zap, Save, Loader2, CheckCircle,
  AlertCircle, Info
} from "lucide-react";
...
export default function AlertProfileEditor({ childId, initialProfile }: AlertProfileEditorProps) {
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      // Note: refactored updateAlertProfile only takes childId and profile now
      const res = await updateAlertProfile(childId, profile);
      if (res.success) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-sm shadow-zinc-200/50 dark:shadow-none">
      <div className="p-8 border-b border-zinc-50 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-800/10">
        <h3 className="text-lg font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-3">
          ...
          Cấu hình Hỗ trợ VR (Response Thresholds)
        </h3>
        <button 
          onClick={handleSave}
          disabled={loading}
          className="bg-blue-600 dark:bg-blue-500 text-white px-6 py-3 rounded-2xl text-xs font-black flex items-center gap-2 hover:scale-105 transition-all shadow-xl shadow-blue-500/20 active:scale-95 disabled:opacity-50"
        >
      ...
      <div className="p-8 space-y-10">
        ...
        <div className="space-y-8">
          <h4 className="text-[10px] font-black text-zinc-400 uppercase tracking-widest border-b border-zinc-100 dark:border-zinc-800 pb-3 flex items-center gap-2">
            <Zap size={14} className="text-amber-500" /> Ngưỡng thời gian phản hồi (Seconds)
          </h4>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-10">
            <ThresholdSlider 
              label="Gợi ý khi đứng hình"
            ...
            />
            <ThresholdSlider 
              label="Hỗ trợ tương tác"
              description="Gợi ý hành động nếu bé chưa tương tác"
  ...
  return (
    <div 
      ...
      className={`p-6 rounded-[2rem] border cursor-pointer transition-all duration-300 ${
        isEnabled 
          ? 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 shadow-xl ring-2 ring-blue-500/10' 
          : 'bg-zinc-50 dark:bg-zinc-900/50 border-zinc-100 dark:border-zinc-800 opacity-60 grayscale scale-95'
      }`}
    >
      <div className="flex justify-between items-center mb-4">
        <div className={`w-2.5 h-2.5 rounded-full shadow-lg ${getSeverityColor()}`} />
        <div className={`w-10 h-5 rounded-full relative transition-colors ${isEnabled ? 'bg-blue-600' : 'bg-zinc-300 dark:bg-zinc-700'}`}>
          <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${isEnabled ? 'right-0.5' : 'left-0.5'}`} />
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        ...
        <span className="text-base font-black text-blue-600 px-3 py-1 bg-blue-50 dark:bg-blue-500/10 rounded-lg">{value}s</span>
      </div>
      <input 
        type="range" 
        min={min} 
